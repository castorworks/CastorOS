# 阶段 14: IBM ThinkPad T41 显卡驱动

## 概述

本阶段为 IBM ThinkPad T41 笔记本电脑实现图形显示支持。T41 系列配备 ATI Mobility Radeon 7500/9000 显卡，支持两种屏幕配置：

- **XGA 型号**：1024×768 分辨率
- **SXGA+ 型号**：1400×1050 分辨率

本阶段实现了从 VGA 文本模式（80×25）到图形模式的完整升级，并**自动适配不同分辨率的屏幕**。

**📝 设计理念**：

本阶段采用**渐进式图形支持**架构：

✅ **PCI 总线驱动** - 已实现
   - PCI 配置空间访问
   - 设备枚举与资源分配
   - 为显卡和其他 PCI 设备提供统一访问接口

✅ **VESA VBE 帧缓冲** - 已实现
   - 通过 GRUB 设置图形模式
   - Multiboot 信息获取帧缓冲地址
   - 硬件无关的基础图形模式

✅ **帧缓冲抽象层** - 已实现
   - 统一的图形操作接口
   - 像素绘制、矩形填充、位图传输
   - 终端仿真（替代 VGA 文本模式）
   - ANSI 转义序列支持

✅ **性能优化** - 已实现
   - PAT (Page Attribute Table) Write-Combining 支持
   - 软件双缓冲优化滚屏性能

⬜ **ATI Radeon 基础驱动**（可选，未实现）
   - 直接访问显卡寄存器
   - 模式设置（Modesetting）
   - 2D 加速操作

---

## 目标完成情况

- [x] 实现 PCI 总线枚举和配置空间访问
- [x] 实现 VESA VBE 图形模式设置（通过 GRUB）
- [x] 实现帧缓冲驱动抽象层
- [x] 实现基础图形操作（像素、线条、矩形、字符渲染）
- [x] 支持多分辨率：1024×768 和 1400×1050（32bpp）
- [x] 实现 EDID 解析（读取功能需要 I2C 硬件支持）
- [x] 实现图形终端仿真（替代 VGA 文本模式）
- [x] 实现 ANSI 转义序列支持（颜色、光标、清屏）
- [x] 实现 PAT Write-Combining 性能优化
- [x] 实现软件双缓冲优化
- [ ] （可选）ATI Radeon 7500/9000 原生驱动

---

## 实现架构

### 整体架构图

```
┌─────────────────────────────────────────────────────────────────┐
│                        应用层                                    │
│   ┌─────────────────┐  ┌─────────────────┐  ┌────────────────┐  │
│   │   kernel_shell  │  │   user/shell    │  │   kprintf      │  │
│   └────────┬────────┘  └────────┬────────┘  └───────┬────────┘  │
└────────────┼────────────────────┼───────────────────┼───────────┘
             │                    │                   │
┌────────────▼────────────────────▼───────────────────▼───────────┐
│                     终端抽象层                                   │
│   ┌─────────────────────────────────────────────────────────┐   │
│   │              fb_terminal_putchar()                      │   │
│   │        支持 ANSI 转义序列 / VGA 颜色兼容                  │   │
│   └─────────────────────────────────────────────────────────┘   │
└─────────────────────────────┬───────────────────────────────────┘
                              │
┌─────────────────────────────▼───────────────────────────────────┐
│                     帧缓冲驱动层                                 │
│   ┌──────────────┐  ┌──────────────┐  ┌──────────────────────┐  │
│   │ 基础绘图函数  │  │  文本渲染    │  │    双缓冲支持        │  │
│   │ fb_put_pixel │  │ fb_draw_char │  │ back_buffer_mem      │  │
│   │ fb_fill_rect │  │ fb_draw_str  │  │ fb_flush()           │  │
│   └──────────────┘  └──────────────┘  └──────────────────────┘  │
└─────────────────────────────┬───────────────────────────────────┘
                              │
┌─────────────────────────────▼───────────────────────────────────┐
│                     内存映射层                                   │
│   ┌─────────────────────────────────────────────────────────┐   │
│   │              vmm_map_framebuffer()                      │   │
│   │        PAT Write-Combining 模式 (高性能)                 │   │
│   └─────────────────────────────────────────────────────────┘   │
└─────────────────────────────┬───────────────────────────────────┘
                              │
┌─────────────────────────────▼───────────────────────────────────┐
│                     硬件层                                       │
│   ┌──────────────────────────────────────────────────────────┐  │
│   │              线性帧缓冲（显存）                            │  │
│   │        物理地址由 Multiboot 信息提供                      │  │
│   └──────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
```

### 输出路径对比

| 场景 | VGA 文本模式 | 图形模式 |
|------|-------------|---------|
| 内核 kprintf | vga_putchar() | fb_terminal_putchar() |
| 内核 shell 颜色 | vga_set_color() | fb_terminal_set_vga_color() |
| 用户态 print() | devconsole → vga_putchar() | devconsole → fb_terminal_putchar() |
| ANSI 颜色 | 不支持 | fb_handle_sgr() |

---

## 已实现功能详解

### 1. PCI 总线驱动

**文件**：
- `src/include/drivers/pci.h`
- `src/drivers/pci.c`

**核心功能**：

```c
// 配置空间访问
uint32_t pci_read_config32(uint8_t bus, uint8_t slot, uint8_t func, uint8_t offset);
void pci_write_config32(uint8_t bus, uint8_t slot, uint8_t func, uint8_t offset, uint32_t value);

// 设备枚举
int pci_scan_devices(void);
pci_device_t *pci_find_device(uint16_t vendor_id, uint16_t device_id);
pci_device_t *pci_find_class(uint8_t class_code, uint8_t subclass);

// BAR 解析
uint32_t pci_get_bar_address(pci_device_t *dev, int bar_index);
uint32_t pci_get_bar_size(pci_device_t *dev, int bar_index);

// 设备控制
void pci_enable_bus_master(pci_device_t *dev);
void pci_enable_memory_space(pci_device_t *dev);
```

**Shell 命令**：
- `lspci` - 列出所有 PCI 设备

### 2. 帧缓冲驱动

**文件**：
- `src/include/drivers/framebuffer.h`
- `src/drivers/framebuffer.c`

**核心功能**：

```c
// 初始化（从 Multiboot 信息）
int fb_init(multiboot_info_t *mbi);

// 基础绘图
void fb_clear(color_t color);
void fb_put_pixel(int x, int y, color_t color);
void fb_fill_rect(int x, int y, int width, int height, color_t color);
void fb_draw_line(int x1, int y1, int x2, int y2, color_t color);

// 文本渲染
void fb_draw_char(int x, int y, char c, color_t fg, color_t bg);
void fb_draw_string(int x, int y, const char *str, color_t fg, color_t bg);

// 终端仿真
void fb_terminal_init(void);
void fb_terminal_putchar(char c);    // 支持 ANSI 转义序列
void fb_terminal_write(const char *str);
void fb_terminal_set_vga_color(uint8_t fg, uint8_t bg);  // VGA 颜色兼容

// 双缓冲
void fb_enable_double_buffer(void);
void fb_flush(void);
void fb_flush_all(void);
```

**支持的 ANSI 转义序列**：

| 序列 | 功能 | 示例 |
|------|------|------|
| `\033[m` | SGR 颜色设置 | `\033[31m` 红色前景 |
| `\033[2J` | 清屏 | `\033[2J\033[H` 清屏并归位 |
| `\033[H` | 光标定位 | `\033[10;20H` 移到第10行第20列 |
| `\033[A/B/C/D` | 光标移动 | `\033[5A` 上移5行 |

**VGA 16 色调色板支持**：

```c
// 标准 VGA 16 色映射到 RGB
static const color_t vga_palette[16] = {
    {0, 0, 0, 255},         // 0: BLACK
    {0, 0, 170, 255},       // 1: BLUE
    {0, 170, 0, 255},       // 2: GREEN
    {0, 170, 170, 255},     // 3: CYAN
    {170, 0, 0, 255},       // 4: RED
    {170, 0, 170, 255},     // 5: MAGENTA
    {170, 85, 0, 255},      // 6: BROWN
    {170, 170, 170, 255},   // 7: LIGHT_GREY
    {85, 85, 85, 255},      // 8: DARK_GREY
    {85, 85, 255, 255},     // 9: LIGHT_BLUE
    {85, 255, 85, 255},     // 10: LIGHT_GREEN
    {85, 255, 255, 255},    // 11: LIGHT_CYAN
    {255, 85, 85, 255},     // 12: LIGHT_RED
    {255, 85, 255, 255},    // 13: LIGHT_MAGENTA
    {255, 255, 85, 255},    // 14: YELLOW
    {255, 255, 255, 255}    // 15: WHITE
};
```

### 3. 性能优化

#### PAT Write-Combining 支持

**文件**：
- `src/mm/vmm.c` - `vmm_init_pat()`, `vmm_map_framebuffer()`
- `src/include/mm/vmm.h`

**原理**：

帧缓冲内存使用 Write-Combining (WC) 缓存模式，允许 CPU 将多个连续的像素写入合并为一个大的写操作，大幅提升写入性能。

```c
// PAT 初始化（在 vmm_init 后调用）
void vmm_init_pat(void);

// 帧缓冲专用映射（使用 WC 模式）
uint32_t vmm_map_framebuffer(uint32_t phys_addr, uint32_t size);
```

**PAT 配置**：
```
PAT[0] = WB  (Write-Back，普通内存)
PAT[1] = WT  (Write-Through)
PAT[2] = UC- (Uncacheable)
PAT[3] = UC  (Uncacheable)
PAT[4] = WB  (Write-Back)
PAT[5] = WT  (Write-Through)
PAT[6] = UC- (Uncacheable)
PAT[7] = WC  (Write-Combining，用于帧缓冲) ← 使用此模式
```

#### 软件双缓冲

**原理**：

在普通内存（WB 模式）中维护后备缓冲区，所有绘制操作在后备缓冲区进行。滚屏等需要读取像素的操作在 WB 内存中执行，速度极快。完成后一次性刷新到显存。

```c
// 双缓冲实现
static uint8_t *back_buffer_mem = NULL;   // 后备缓冲区（WB 内存）
static bool double_buffering = false;
static int dirty_line_start = -1;          // 脏区域追踪
static int dirty_line_end = -1;

// 刷新策略
void fb_flush(void) {
    // 只刷新脏区域，减少不必要的显存写入
    if (dirty_line_start >= 0 && dirty_line_end >= 0) {
        uint32_t offset = dirty_line_start * fb_info.pitch;
        uint32_t size = (dirty_line_end - dirty_line_start) * fb_info.pitch;
        memcpy(fb_info.buffer + offset, back_buffer_mem + offset, size);
    }
}
```

**自动刷新时机**：
- 换行时刷新（用户能看到输出）
- 滚屏后刷新
- 字符串输出完成后刷新

**性能对比**：

| 操作 | 优化前 | 优化后 |
|------|--------|--------|
| 像素写入 | UC 显存（无缓存） | WC 显存（写合并）或 WB 后备缓冲区 |
| 滚屏读取 | UC 显存（极慢） | WB 后备缓冲区（缓存命中） |
| 滚屏写入 | UC 显存 | WB 后备缓冲区 + 批量刷新 |

### 4. EDID 支持

**文件**：
- `src/include/drivers/edid.h`
- `src/drivers/edid.c`

**已实现功能**：
- EDID 数据块验证
- EDID 解析（制造商、分辨率、尺寸等）
- 首选分辨率提取

**未实现功能**：
- I2C/DDC 硬件读取（需要显卡特定的 GPIO 位操作）

```c
// 解析 EDID 数据（已实现）
int edid_parse(const uint8_t *data, edid_info_t *info);
bool edid_validate(const uint8_t *data);
void edid_print_info(const edid_info_t *info);

// 从硬件读取（框架已实现，I2C 位操作未实现）
int edid_read_from_radeon(volatile uint32_t *mmio_base, edid_info_t *info);
```

### 5. GRUB 配置

**文件**：`grub.cfg`

```bash
# 多分辨率支持配置
# GRUB 会按顺序尝试，选择第一个支持的分辨率
set gfxmode=1400x1050x32,1024x768x32,800x600x32,auto
set gfxpayload=keep

# 启动菜单
menuentry "CastorOS (Auto Resolution)" {
    set gfxpayload=keep
    multiboot /boot/castor.bin
    boot
}

menuentry "CastorOS (1400x1050 SXGA+)" { ... }
menuentry "CastorOS (1024x768 XGA)" { ... }
menuentry "CastorOS (Text Mode)" { ... }
```

### 6. Multiboot 头部

**文件**：`src/boot/multiboot.asm`

```nasm
; 视频模式字段（请求图形模式）
dd 0                            ; mode_type: 0 = 线性图形模式
dd 0                            ; width: 0 = 让 GRUB 根据 gfxpayload 选择
dd 0                            ; height: 0 = 让 GRUB 根据 gfxpayload 选择
dd 32                           ; depth: 首选位深度（32bpp）
```

---

## 内核集成

### kernel.c 初始化流程

```c
void kernel_main(multiboot_info_t* mbi) {
    // ... 早期初始化 ...
    
    // 3.2 初始化 VMM
    vmm_init();
    
    // 3.3 初始化 PAT（用于帧缓冲 Write-Combining）
    vmm_init_pat();
    
    // ... 其他初始化 ...
    
    // 4.5 初始化 PCI 总线
    pci_init();
    pci_scan_devices();
    
    // 4.6 初始化图形驱动
    int fb_result = fb_init(mbi);
    if (fb_result == 0) {
        // 图形模式成功
        fb_terminal_init();  // 初始化终端仿真
        // kprintf 自动使用 fb_terminal_putchar
    } else {
        // 回退到 VGA 文本模式
        // kprintf 使用 vga_putchar
    }
}
```

### kprintf 自动切换

**文件**：`src/lib/kprintf.c`

```c
static void output_char(char c, output_target_t target) {
    if (target & OUTPUT_VGA) {
        // 优先使用图形终端，回退到 VGA 文本模式
        if (fb_is_initialized()) {
            fb_terminal_putchar(c);
        } else {
            vga_putchar(c);
        }
    }
}
```

### devfs 控制台输出

**文件**：`src/fs/devfs.c`

```c
static uint32_t devconsole_write(fs_node_t *node, uint32_t offset,
                                 uint32_t size, uint8_t *buffer) {
    for (uint32_t i = 0; i < size; i++) {
        if (fb_is_initialized()) {
            fb_terminal_putchar(buffer[i]);
        } else {
            vga_putchar(buffer[i]);
        }
    }
    return size;
}
```

---

## Shell 命令

| 命令 | 描述 |
|------|------|
| `lspci` | 列出所有 PCI 设备 |
| `fbinfo` | 显示帧缓冲信息（分辨率、颜色深度、物理地址）|

---

## 技术背景

### IBM ThinkPad T41 显卡规格

**ATI Mobility Radeon 7500** 规格：
- GPU 架构：RV200（R200 系列移动版）
- 显存：16MB/32MB DDR SDRAM
- 核心频率：230-290 MHz
- 接口：AGP 4x
- 最大分辨率：2048×1536（外接）/ 1400×1050（内置 LCD）

**T41 内置显示屏**：
- 尺寸：14.1 英寸
- 分辨率：1024×768（XGA）或 1400×1050（SXGA+）
- 类型：TFT LCD

### 帧缓冲（Framebuffer）概念

**线性帧缓冲**：
- 屏幕像素按行连续存储
- 像素地址 = 基地址 + y × pitch + x × bytes_per_pixel
- 无需处理内存分页（bank switching）

**像素格式**（以 32bpp 为例）：
```
32bpp ARGB (8-8-8-8):
  31-24: Alpha (或保留)
  23-16: Red
  15-8:  Green
  7-0:   Blue
```

---

## 文件清单

### 新增文件

- `src/include/drivers/pci.h` - PCI 总线驱动头文件
- `src/drivers/pci.c` - PCI 总线驱动实现
- `src/include/drivers/framebuffer.h` - 帧缓冲驱动头文件
- `src/drivers/framebuffer.c` - 帧缓冲驱动实现
- `src/include/drivers/edid.h` - EDID 读取驱动头文件
- `src/drivers/edid.c` - EDID 读取驱动实现
- `src/include/drivers/font8x16.h` - 8x16 点阵字体数据

### 修改文件

- `src/boot/multiboot.asm` - 添加图形模式请求
- `src/include/kernel/multiboot.h` - 添加帧缓冲相关字段
- `src/kernel/kernel.c` - 集成图形初始化
- `src/lib/kprintf.c` - 添加图形模式输出支持
- `src/include/lib/kprintf.h` - 添加 kconsole_set_color 声明
- `src/fs/devfs.c` - 控制台设备支持图形模式
- `src/kernel/kernel_shell.c` - shell 颜色兼容层
- `src/mm/vmm.c` - 添加 PAT 和帧缓冲映射支持
- `src/include/mm/vmm.h` - 添加相关声明
- `grub.cfg` - 配置多分辨率图形模式
- `Makefile` - 添加新文件编译规则

---

## 限制和未来改进

### 当前限制

1. 仅支持 Multiboot/GRUB 提供的图形模式
2. 不支持运行时模式切换
3. 不支持硬件加速
4. 字体渲染简单（无抗锯齿）
5. EDID I2C 读取未实现（需要显卡特定代码）

### 未来改进

1. **模式切换**
   - 实现 V86 模式调用 VBE BIOS
   - 支持运行时分辨率切换

2. **ATI 原生驱动**
   - 直接操作 Radeon 寄存器
   - 实现 2D 加速（矩形填充、位块传输）
   - 实现硬件光标

3. **GUI 框架**
   - 窗口管理器
   - 鼠标支持
   - 控件库

4. **高级渲染**
   - 抗锯齿字体（FreeType）
   - Alpha 混合
   - 图片格式支持（BMP、PNG）

---

## 参考资料

1. [OSDev - PCI](https://wiki.osdev.org/PCI)
2. [OSDev - VESA Video Modes](https://wiki.osdev.org/VESA_Video_Modes)
3. [OSDev - Drawing In Protected Mode](https://wiki.osdev.org/Drawing_In_Protected_Mode)
4. [Multiboot Specification](https://www.gnu.org/software/grub/manual/multiboot/multiboot.html)
5. [Intel PAT (Page Attribute Table)](https://wiki.osdev.org/Page_Attribute_Table)
6. [ATI Radeon Register Reference](https://www.x.org/docs/AMD/)
