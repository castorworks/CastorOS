...
uint32_t sys_brk(uint32_t addr) {
    ...
    if (addr == 0) {
        LOG_DEBUG_MSG("sys_brk: returning current heap_end=%x\n", current->heap_end);
    ...
    LOG_DEBUG_MSG("sys_brk: old_end=%x (aligned %x), new_end=%x (aligned %x)\n",
                  old_end, old_end_aligned, addr, new_end_aligned);
    
    if (new_end_aligned > old_end_aligned) {
        // 扩展堆：分配并映射新页面
        LOG_DEBUG_MSG("sys_brk: expanding heap from %x to %x\n", 
        ...
        for (uint32_t page = old_end_aligned; page < new_end_aligned; page += PAGE_SIZE) {
            ...
            if (!phys) {
                LOG_ERROR_MSG("sys_brk: out of memory at page %x\n", page);
                // 不回滚，保持已分配的页面
                // 返回实际达到的地址
            ...
            LOG_DEBUG_MSG("sys_brk: mapped page %x -> phys %x\n", page, phys);
        }
    } else if (new_end_aligned < old_end_aligned) {
        // 收缩堆：取消映射并释放页面
        LOG_DEBUG_MSG("sys_brk: shrinking heap from %x to %x\n", 
                      old_end_aligned, new_end_aligned);
        ...
        for (uint32_t page = new_end_aligned; page < old_end_aligned; page += PAGE_SIZE) {
            ...
            if (phys) {
                pmm_free_frame(phys);
                LOG_DEBUG_MSG("sys_brk: unmapped page %x (phys %x)\n", page, phys);
            }
        }
    }
    ...
    LOG_DEBUG_MSG("sys_brk: heap extended to %x\n", addr);
...
static uint32_t do_mmap_anonymous(task_t *current, uint32_t vaddr, uint32_t length,
    ...
    for (uint32_t page = vaddr; page < vaddr + length; page += PAGE_SIZE) {
        ...
        uint32_t phys = pmm_alloc_frame();
        if (!phys) {
            LOG_ERROR_MSG("sys_mmap: out of memory at page %x\n", page);
            // 回滚已分配的页面
        ...
        if (!vmm_map_page_in_directory(current->page_dir_phys, page, phys, page_flags)) {
            pmm_free_frame(phys);
            LOG_ERROR_MSG("sys_mmap: failed to map page %x\n", page);
            // 回滚已分配的页面
            for (uint32_t p = vaddr; p < page; p += PAGE_SIZE) {
                uint32_t pf = vmm_unmap_page_in_directory(current->page_dir_phys, p);
                if (pf) {
    ...
    LOG_DEBUG_MSG("sys_mmap: anonymous mapped %x bytes at %x (%u pages)\n", 
...
static uint32_t do_mmap_file(task_t *current, uint32_t vaddr, uint32_t length,
    ...
    for (uint32_t page = vaddr; page < vaddr + length; page += PAGE_SIZE) {
        ...
        if (file_offset < file_size) {
            ...
            if (bytes_read == 0 && read_size > 0) {
                LOG_WARN_MSG("sys_mmap: failed to read file at offset %x\n", file_offset);
            }
            
            LOG_DEBUG_MSG("sys_mmap: read %u bytes from file offset %x to page %x\n",
    ...
    LOG_DEBUG_MSG("sys_mmap: file mapped %x bytes at %x (%u pages)\n", 
                  length, vaddr, pages_allocated);
...
uint32_t sys_mmap(uint32_t addr, uint32_t length, uint32_t prot,
    ...
    if (length > MMAP_REGION_END - MMAP_REGION_START) {
        LOG_ERROR_MSG("sys_mmap: length %x too large\n", length);
        return (uint32_t)-1;
    }
    
    // 检查 offset 是否页对齐
    if (offset & (PAGE_SIZE - 1)) {
        LOG_ERROR_MSG("sys_mmap: offset %x not page aligned\n", offset);
        return (uint32_t)-1;
    }
    ...
    LOG_DEBUG_MSG("sys_mmap: addr=%x, length=%x, prot=%x, flags=%x, fd=%d, offset=%x\n",
    ...
    if (vaddr == 0) {
        LOG_ERROR_MSG("sys_mmap: no free virtual address space for length %x\n", length);
        return (uint32_t)-1;
    }
...
uint32_t sys_munmap(uint32_t addr, uint32_t length) {
    ...
    if (aligned_addr >= USER_SPACE_END || aligned_addr + length > USER_SPACE_END) {
        LOG_ERROR_MSG("sys_munmap: address %x out of user space\n", aligned_addr);
        return (uint32_t)-1;
    }
    
    LOG_DEBUG_MSG("sys_munmap: addr=%x, length=%x\n", aligned_addr, length);
