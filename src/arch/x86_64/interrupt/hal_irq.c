/**
 * @file hal_irq.c
 * @brief HAL 逻辑中断号实现 (x86_64)
 * 
 * 实现 x86_64 架构的逻辑中断号到物理 PIC/APIC IRQ 号的映射。
 * 支持 PIC 和 APIC 两种中断控制器。
 * 
...
/* ============================================================================
 * x86_64 PIC/APIC IRQ Mapping
 * 
 * x86_64 使用 8259 PIC 或 APIC，IRQ 0-15 映射到中断向量 32-47
 * 
 * IRQ 0:  Timer (PIT/LAPIC Timer)
 * IRQ 1:  Keyboard
 ...
 * Handler Wrapper
 * 
 * x86_64 的 IRQ 处理函数签名是 void handler(registers_t *regs)
...
hal_error_t hal_irq_register_logical(hal_irq_type_t type, uint32_t instance,
    ...
    irq64_register_handler((uint8_t)phys_irq, wrapper);
...
hal_error_t hal_irq_unregister_logical(hal_irq_type_t type, uint32_t instance) {
    ...
    /* 注销底层 IRQ 处理函数 */
    irq64_register_handler((uint8_t)phys_irq, NULL);
...
hal_error_t hal_irq_enable_logical(hal_irq_type_t type, uint32_t instance) {
    ...
    irq64_enable_line((uint8_t)phys_irq);
    return HAL_OK;
}
...
hal_error_t hal_irq_disable_logical(hal_irq_type_t type, uint32_t instance) {
    ...
    irq64_disable_line((uint8_t)phys_irq);
    return HAL_OK;
}
