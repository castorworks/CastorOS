...
TEST_CASE(test_vfs_create_file) {
    ...
    // 创建测试文件
    int result = vfs_create("/test_file_create");
    ASSERT_EQ(result, 0);
    
    // 验证文件存在
    fs_node_t *node = vfs_path_to_node("/test_file_create");
    ASSERT_NOT_NULL(node);
    ASSERT_EQ(node->type, FS_FILE);
    
    // 清理
    vfs_release_node(node);
    vfs_unlink("/test_file_create");
}
...
TEST_CASE(test_vfs_read_write) {
    ...
    // 创建测试文件
    int result = vfs_create("/test_rw_file");
    ASSERT_EQ(result, 0);
    
    // 获取文件节点
    fs_node_t *node = vfs_path_to_node("/test_rw_file");
    ASSERT_NOT_NULL(node);
...
TEST_CASE(test_vfs_read_write_offset) {
    ...
    // 创建测试文件
    int result = vfs_create("/test_offset_file");
    ASSERT_EQ(result, 0);
    
    fs_node_t *node = vfs_path_to_node("/test_offset_file");
    ASSERT_NOT_NULL(node);
...
TEST_CASE(test_vfs_unlink_file) {
    ...
    // 创建测试文件
    int result = vfs_create("/test_unlink_file");
    ASSERT_EQ(result, 0);
    
    // 验证文件存在
    fs_node_t *node = vfs_path_to_node("/test_unlink_file");
    ASSERT_NOT_NULL(node);
    vfs_release_node(node);
    
    // 删除文件
    result = vfs_unlink("/test_unlink_file");
    ASSERT_EQ(result, 0);
    
    // 验证文件不存在
    node = vfs_path_to_node("/test_unlink_file");
    ASSERT_NULL(node);
}
...
TEST_CASE(test_vfs_read_empty_file) {
    ...
    // 创建空文件
    int result = vfs_create("/test_empty_file");
    ASSERT_EQ(result, 0);
    
    fs_node_t *node = vfs_path_to_node("/test_empty_file");
    ASSERT_NOT_NULL(node);
...
TEST_CASE(test_vfs_mkdir) {
    ...
    // 创建测试目录
    int result = vfs_mkdir("/test_dir", FS_PERM_READ | FS_PERM_WRITE);
    ASSERT_EQ(result, 0);
    
    // 验证目录存在
    fs_node_t *node = vfs_path_to_node("/test_dir");
    ASSERT_NOT_NULL(node);
...
TEST_CASE(test_vfs_create_in_subdir) {
    ...
    // 创建子目录
    int result = vfs_mkdir("/test_subdir", FS_PERM_READ | FS_PERM_WRITE);
    ASSERT_EQ(result, 0);
    
    // 在子目录中创建文件
    result = vfs_create("/test_subdir/subfile");
    ASSERT_EQ(result, 0);
    
    // 验证文件存在
    fs_node_t *node = vfs_path_to_node("/test_subdir/subfile");
    ASSERT_NOT_NULL(node);
    ASSERT_EQ(node->type, FS_FILE);
    vfs_release_node(node);
    
    // 清理
    vfs_unlink("/test_subdir/subfile");
    vfs_unlink("/test_subdir");
}
...
TEST_CASE(test_vfs_finddir) {
    ...
    // 创建测试文件
    int result = vfs_create("/test_finddir_file");
    ASSERT_EQ(result, 0);
    ...
    fs_node_t *found = vfs_finddir(root, "test_finddir_file");
    ASSERT_NOT_NULL(found);
    ASSERT_EQ(found->type, FS_FILE);
    vfs_release_node(found);
    
    // 查找不存在的文件
    found = vfs_finddir(root, "nonexistent_file");
    ASSERT_NULL(found);
    
    // 清理
    vfs_unlink("/test_finddir_file");
}
...
TEST_CASE(test_vfs_readdir) {
    ...
    // 创建测试目录和文件
    vfs_mkdir("/test_readdir", FS_PERM_READ | FS_PERM_WRITE);
    vfs_create("/test_readdir/file1");
    vfs_create("/test_readdir/file2");
    
    // 获取目录节点
    fs_node_t *dir = vfs_path_to_node("/test_readdir");
    ASSERT_NOT_NULL(dir);
    ...
    vfs_unlink("/test_readdir/file1");
    vfs_unlink("/test_readdir/file2");
    vfs_unlink("/test_readdir");
}
...
TEST_CASE(test_vfs_create_duplicate) {
    ...
    // 创建文件
    int result = vfs_create("/test_dup_file");
    ASSERT_EQ(result, 0);
    
    // 尝试再次创建同名文件
    result = vfs_create("/test_dup_file");
    ASSERT_EQ(result, -1);  // 应该失败
    
    // 清理
    vfs_unlink("/test_dup_file");
}
...
TEST_CASE(test_vfs_unlink_nonempty_dir) {
    ...
    // 创建目录和文件
    vfs_mkdir("/test_nonempty", FS_PERM_READ | FS_PERM_WRITE);
    vfs_create("/test_nonempty/file");
    
    // 尝试删除非空目录
    int result = vfs_unlink("/test_nonempty");
    ASSERT_EQ(result, -1);  // 应该失败
    
    // 清理
    vfs_unlink("/test_nonempty/file");
    vfs_unlink("/test_nonempty");
}
