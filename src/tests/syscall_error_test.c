...
PBT_PROPERTY(invalid_fd_returns_ebadf) {
    ...
    if (pbt_gen_bool(state)) {
        // Negative FD
        invalid_fd = -(int32_t)pbt_gen_uint32_range(state, 1, 1000);
    } else {
        // Very large FD (beyond typical limit)
        invalid_fd = (int32_t)pbt_gen_uint32_range(state, 10000, 100000);
    }
...
PBT_PROPERTY(invalid_syscall_returns_error) {
    ...
    uint32_t invalid_syscall = pbt_gen_uint32_range(state, SYS_MAX, SYS_MAX + 1000);
...
PBT_PROPERTY(error_codes_in_valid_range) {
    ...
    uint32_t idx = pbt_gen_choice(state, num_codes);
...
PBT_PROPERTY(negative_errno_roundtrip) {
    ...
    int32_t errno_val = (int32_t)pbt_gen_uint32_range(state, 1, MAX_ERRNO);
...
PBT_PROPERTY(posix_error_codes_consistent) {
    ...
    uint32_t choice = pbt_gen_choice(state, 10);
...
PBT_PROPERTY(success_not_error) {
    ...
    int32_t success_val = (int32_t)pbt_gen_uint32_range(state, 0, 0x7FFFFFFF);
