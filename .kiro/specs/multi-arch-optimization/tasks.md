...
- [ ] 3. 实现页表抽象接口
  - [ ] 3.1 创建 `src/include/hal/pgtable.h` 定义页表抽象接口
...
- [ ] 5. 重构 VMM 使用页表抽象
  - [ ] 5.1 修改 `src/mm/vmm.c` 使用 pgtable 抽象
...
- [ ] 9. 实现平台设备框架
  - [ ] 9.1 创建 `src/include/drivers/platform.h` 定义平台设备接口
  ...
  - [ ] 9.2 实现平台设备核心框架
    ...
    - 实现驱动注册和设备匹配逻辑
    - _Requirements: 6.1_
  - [ ] 9.3 实现 PCI 设备到平台设备的转换
    - 在 `src/drivers/platform/pci_platform.c` 中实现
    - 从 PCI 配置空间提取资源信息
    - _Requirements: 6.2_
  - [ ] 9.4 实现 DTB 设备到平台设备的转换
...
- [ ] 15. 完善 HAL 接口实现
  - [ ] 15.1 实现 i686 缺失的 HAL 函数
    - 添加 `hal_arch_name()` 实现
    - 确保所有 HAL 函数有实现或 stub
    - _Requirements: 2.1, 2.4_
  - [ ] 15.2 实现 x86_64 缺失的 HAL 函数
    - 完善 `hal_context_init()` 实现
    - 确保所有 HAL 函数有实现或 stub
    - _Requirements: 2.2, 2.3_
  - [ ] 15.3 实现 arm64 缺失的 HAL 函数
    - 完善 `hal_context_init()` 实现
    - 确保所有 HAL 函数有实现或 stub
    - _Requirements: 2.2, 2.3_
